...
function rcp_process_lostpassword_form() {
	...
	if ( ! is_wp_error( $errors ) ) {
		$redirect_to = wp_validate_redirect( wp_unslash( $_POST['rcp_redirect'] ), home_url() );
...
function rcp_retrieve_password() {
	...
	$redirect_base = wp_validate_redirect( wp_unslash( $_POST['rcp_redirect'] ), home_url() );
	$message .= esc_url_raw( add_query_arg( array( 'rcp_action' => 'lostpassword_reset', 'key' => $key, 'login' => rawurlencode( $user_login ) ), $redirect_base ) ) . "\r\n";
